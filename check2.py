...
class Server(object):
    ...
    def turn(self, msg):
        ...
        output = msg + " yourTurn\n"
        name = self.decode_name_from_clients(msg)
        for clients in self.connection_lst:
            if clients == client and self.FirstPlayerInLine == '' or name == self.FirstPlayerInLine:
                self.FirstPlayerInLine = self.decode_name_from_clients(msg)
                clients.send(output.encode())
            else:
                output = msg + " no"
                clients.send(output.encode())
    ...
    def handle_client_connection(self, client_socket, current):
        ...
        while True:
            ...
            if "playerInfo" in msg and self.count > 1:
