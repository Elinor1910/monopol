...
class Client(object):
    ...
    def handle_client(self):
        # self.sock.send(str(output).encode())
        while True:
            msg = self.sock.recv(1024).decode()
            print(msg)
            if 'player count' in msg:
                if '1' in msg:
                    wait = tk.Label(root, text='Waiting for more players to join....', bg='white')
                    wait.place(x=200, y=100)
                if '2' in msg:
                    register = str(self.name) + "|" + "0" + "|" + str(self.color)
    ...
    def decode_server_msg(self, info):
        ...
        count += 1
        while info[count] != '|':
            psum = psum + int(info[count])
            count += 1
        # finding color
        x = count
        count = len(info) - 1
        while info[count] != x:
            color = color + info[count]
            x += 1
        print("name: " + name + "psum: " + str(psum) + "color: " + color)
        return place, name, psum, color
    ...
    def draw_board(self, first, msg):
        ...
        self.draw_players(msg, window)
        self.players_actions(msg, window)
        self.update_and_show_money(window)
        self.cube(window)
        if not first:
            cube = tk.Label(window, bg='white', width=11, height=6)
            cube.place(x=1000, y=350)
        window.mainloop()

    def players_actions(self, msg, window):
        place, name, psum, color = self.decode_server_msg(msg)
        x, y, bp = self.finding_player_place(psum)
        if bp == "surprise":
            self.gift_cards(window)
        if bp == "question":
    ...
    def draw_players(self, msg, window):
        place, name, psum, color = self.decode_server_msg(msg)
        if place == 'first':
            player = tk.Label(window, text=name, bg=color, fg='white')
            player.place(x=int(x), y=int(y))
        elif place == 'second':
            player = tk.Label(window, text=name, bg=color, fg='white')
